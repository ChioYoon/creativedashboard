...
**작성일:** 2026-07-02
**대상:** `index.html`, `step1_integrated.html`, `live_dashboard.html`, `assets/ds/colors_and_type.css`
...
**토큰 값 비교 결과**(직접 diff): step1·index가 로컬로 복제한 토큰명은 전부 DS 파일에 동일 이름으로 존재하며, 값도 거의 100% 일치한다. 예외 2건:
1. `index.html`의 `--text-muted: #999999`(1곳 사용) — DS 파일엔 이 시맨틱 별칭이 없음(원시 램프값 `--gray-30: #999999`만 존재).
2. `--font-kr` 폴백 체인 — index/step1 로컬 버전엔 `"Pretendard"`·`"Malgun Gothic"` 폴백이 일부 빠져있음(DS 파일 쪽이 더 완전함). 웹폰트(Noto Sans KR)가 항상 우선 로드되므로 실사용상 시각적 차이는 없음.

## 설계 — 3개 페이지를 공식 DS 파일로 통일

### 1. `assets/ds/colors_and_type.css` (1줄 추가)

시맨틱 섹션(`--text-disabled` 인접)에 추가:
```css
...
`colors_and_type.css` 내용이 바뀌므로(§1), 이 파일을 로드하는 **5개 페이지 전부** 버전 쿼리를 `?v=20260702a`로 부여/통일:
- `index.html`(신규), `step1_integrated.html`(신규), `live_dashboard.html`(신규) — 위 §2~4에서 추가하며 함께 부여
...
각 페이지 로드 후:
1. 콘솔 error 0.
2. `getComputedStyle`로 대표 요소 확인 — 브랜드 레드 배경(`.cl-nav-cta`/`.live-metric-btn.active` 등)이 `rgb(220, 40, 40)`, 카드 배경(`.ds-content-card`류 또는 각 페이지 카드)이 변경 전과 동일한지.
3. index.html: `--text-muted` 참조 위치가 여전히 `#999999`(=`--gray-30`)로 렌더링되는지.
4. step1_integrated.html: `.ds-topbar` 관련 DOM/CSS 잔존 참조 0건 재확인.
5. `pytest` 무회귀(CSS/HTML 변경만이라 영향 없을 것으로 예상, 그래도 실행).
