...
14. cta_type — CTA(행동 유도) 설계 유형 1개. 명시적 CTA(버튼/자막/나레이션의 '사전예약·다운로드' 등)가 없으면 null:
   · "보상 보증형" — 쿠폰코드·구체 보상 조건을 화면에 명시해 유입 유도
   · "마켓 버튼 모사형" — 스토어 설치/다운로드 버튼 UI 그래픽을 강하게 노출
   · "상태 변화 유도형" — 진행도·성공률('강화 99%') 등 완료 심리를 자극
   · "로고 정적 노출형" — 혜택 없이 로고·CI만 노출하고 종료하는 엔드카드
