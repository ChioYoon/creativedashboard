...
# 실 API 형식: result.actuals.data.rows[], row = {groupBys:[creative,channel,campaign,event_date], values:{key:{value}}}
# groupBys 순서: ["ad_creative", "channel", "campaign", "event_date"] — gb[3]=event_date(일자별 분해)
TEST_DATE = "2026-02-10"
DAY1, DAY2 = "2026-02-09", "2026-02-10"
RESULT = {"actuals": {"data": {"rows": [
    # 동일 소재×채널×캠페인이 일자별로 2행 — event_date 가 gb[3] 에서 읽혀야 함
    {"groupBys": ["260123_VID_A-AI-Pinball01A-FK_V_1080x1920_EN", "facebook.business",
                  "HQ_HQ_PH_CA-EN_FB_NU_AD_INSTALL_251211", DAY1],
    ...
    {"groupBys": ["260123_VID_A-AI-Pinball01A-FK_V_1080x1920_EN", "facebook.business",
                  "HQ_HQ_PH_CA-EN_FB_NU_AD_INSTALL_251211", DAY2],
     "values": {"impressions_channel": {"value": 3000.0}, "clicks_channel": {"value": 80.0},
                "cost_channel": {"value": 500.0}, "app_installs": {"value": 60.0}}},
...
assert len(rows) == 2, f"기대 2행(facebook 2일치), 실제 {len(rows)}"
d = rows[0]
assert d.creative_name == "260123_VID_A-AI-Pinball01A-FK_V_1080x1920_EN"
assert d.channel == "facebook.business" and d.date == DAY1  # ★ event_date(gb[3]) 사용 — default_date 아님
...
assert rows[1].date == DAY2 and rows[1].impressions == 3000  # 둘째 날 분리 저장

# event_date 미포함(3-element) → default_date 폴백 (하위호환)
LEGACY = {"actuals": {"data": {"rows": [
    {"groupBys": ["260123_VID_A-AI-Pinball01A-FK_V_1080x1920_EN", "facebook.business",
                  "HQ_HQ_PH_CA-EN_FB_NU_AD_INSTALL_251211"],
     "values": {"impressions_channel": {"value": 100.0}}},
]}}}
legacy_rows = parse_actuals_rows(LEGACY, DEFAULT_METRICS, exclude_channels=set(), default_date=TEST_DATE)
assert legacy_rows[0].date == TEST_DATE, "event_date 없으면 default_date 폴백"
