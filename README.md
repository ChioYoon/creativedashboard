...
### 🎯 소재 제작 자동화 (다음 대형 목표 — 기획 단계)
생성형 AI 기반 소재 자동 생성으로 확장. 원칙: 작게→검증→확장, 단계별 go/no-go, 초기엔 사람 승인(추천형).
- [ ] **0. 생성 툴 조사**: 이미지/영상 생성 툴(Higgsfield/Seedance/매체 내 AI/PlayAd 등) — 비용·API·에셋 학습·상업 사용권
- [ ] **1. 수동 파일럿**: 위닝 소재 3~5개로 AI 배너 수동 생성·품질 검증 (자동화 착수 관문, 코드 없음)
- [ ] **2. 표준 브리프 자동 출력**: 대시보드가 소재별 태그·개선 액션·게임 컨텍스트를 생성 AI용 브리프 규격으로 산출
- [ ] **3. 생성 연동**: 툴 API 연동 → 브리프→생성 자동 + 검수 큐·사람 승인
- [ ] **4. 운영화**: 배치·A/B 변주 + 생성 소재 성과를 스코어링에 피드백
- [ ] **소재 자동 최적화(매체 on/off)**: 추천형(사람 승인)은 기존 데이터로 가능 / 완전 자동은 Google Ads write 연동 필요(resource_name 저장·developer token 레벨 확인)

### 분석 기능
- [ ] **Jaccard 유사도 기반 동적 군집화**: 현재 Union-Find를 Jaccard 유사도 기반으로 교체
- [ ] **BNR/VID 다차원 분석**: 군집별 배너/비디오 성과 차이 및 최적화 포인트 제시
- [ ] **성과 견인 핵심 태그 자동 추출**: 상위 스코어 군집 공통 결정 태그 식별
- [ ] **소재별 시계열 성과 트렌드**: 소재 단위 기간 추이(피로도와 별개)
- [ ] 다국가/매체별 성과 필터 확장
- [x] ✅ 소재 피로도 분석 (기간 비교·상태 판정 — `step1_integrated.html`에 내장, 별도 페이지 아님)
- [x] ✅ 날짜 범위 필터 · 캠페인/캐노니컬 5차원 필터 · 매체 축 정규화

### UI/UX
- [ ] **상단 맥락 바 이전**: 타이틀·레이어·기간을 사이드바→상단 상시 바로 (레이아웃 재편 — 전용 작업)
- [x] ✅ 실시간 반영(실행 버튼 제거) · 가중치 프리셋 · 요약-우선 홈(라이브 대시보드) · 점수 명칭 통일

### 데이터 연동 / 리포트
- [x] ✅ Google Ads + MMP(Airbridge/AppsFlyer) **수집 연동** + nightly 자동 갱신 + 결과 이메일 발송 (`pipeline/`)
- [ ] Google Ads **write**(소재 on/off 실행) 연동 — 자동 최적화 완전 자동화 시
- [ ] PDF/PPT 자동 내보내기 (현재 HTML 추출은 지원)
- [ ] 서버사이드 REST API (현재는 정적 + nightly 배치)
